...
const PAGE_ASSESS = {
  'PAGE-001': {
    product: 'FAIL',
    task_id: 'RUNTIME-FILTER-001',
    root_cause_id: 'RC-DEVICE-FILTER',
    first_breakpoint: {
      target: 'FEAT-014',
      test: 'TEST-U-007',
      file: 'apps/uniapp/services/ble-runtime/device-filter.js',
      symbol: 'filterBleDevices',
      breakpoint: 'prefix startsWith only; missing keyword/token hit matching (REQ-014)',
    },
    also: [
      { task_id: 'RUNTIME-DISPLAY-NAME-001', file: 'apps/uniapp/services/ble-runtime/display-name.js', breakpoint: 'module missing' },
    ...
    notes: 'Fake Runtime E4 PASS；产品扫描过滤/显示名/时长未对齐目标',
  },
  'PAGE-002': {
    product: 'FAIL',
  ...
  'PAGE-007': {
    product: 'FAIL',
    task_id: 'RUNTIME-SESSION-001',
    root_cause_id: 'RC-SESSION-REGISTRY',
    first_breakpoint: {
      target: 'REQ-053',
      test: 'TEST-I-009',
      file: 'apps/uniapp/services/ble-runtime/index.js',
      symbol: 'Registry',
      breakpoint: 'no provisioning-session classify/exclude API for PAGE-007 list',
    },
    also: [
      { task_id: 'RUNTIME-SESSION-001', breakpoint: 'subscription_count missing (TEST-U-005 / DEC-017)' },
    ],
    notes: 'connected/index 存在；Session 口径未对齐',
  },
  'PAGE-008': {
    product: 'FAIL',
...
function renderRemediation(summaryByPage) {
  const order = [
    ...
    ['PAGE-001', 'RUNTIME-FILTER-001', 'P1', 'device-filter 关键词命中'],
    ['PAGE-001', 'RUNTIME-DISPLAY-NAME-001', 'P1', 'display-name.js 缺失'],
...
- PAGE-001 → RUNTIME-FILTER-001 + RUNTIME-DISPLAY-NAME-001（勿建 PAGE-SCAN-FIX）
...
| PAGE-SESSION-001 | RUNTIME-SESSION-001 |
