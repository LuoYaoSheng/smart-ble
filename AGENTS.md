...
- `cd apps/uniapp && npm install && npm run build:h5` compiles the H5 preview via the pure CLI toolchain (`@dcloudio` vite line, no HBuilderX); `npm run build:app` compiles APP resources. Output stays under `apps/uniapp/unpackage/dist/`. HBuilderX is only needed for device launch / custom base / APK packaging. The WeChat mini-program target (`build:mp-weixin`) was retired on 2026-09-11 by user decision and must not come back.
