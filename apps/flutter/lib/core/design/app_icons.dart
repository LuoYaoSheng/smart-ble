...
const List<String> kAppIconNames = ['scan', 'bt', 'link', 'cast', 'info', 'share', 'chev-r', 'chev-d', 'check', 'x', 'warn', 'copy', 'refresh', 'play', 'stop', 'qr', 'wifi', 'usb', 'send', 'eye', 'eye-off', 'trash', 'chip', 'log', 'set', 'lock', 'doc', 'ext', 'hid', 'pulse', 'folder', 'dl', 'box', 'signal', 'batt'];

const Map<String, String> kAppIcons = {
...
/// 正典图标组件：name 取 [kAppIconNames]；color 缺省跟随当前文本色；
/// rotate 用于正典单字形的方向变体（如返回箭头 = chev-r 旋转 180°）。
class AppIcon extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final body = (kAppIcons[name] ?? kAppIcons['info']!)
        .replaceAll('{C}', hex)
        .replaceAll('{KNOB}', knob);
    final g = rotate == 0 ? '' : '<g transform="rotate($rotate 12 12)">';
    final gEnd = rotate == 0 ? '' : '</g>';
    return SvgPicture.string(
      '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">$g$body$gEnd</svg>',
      width: size,
      height: size,
    );
  }
