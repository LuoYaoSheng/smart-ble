...
// TEST-E-006/008（静态前置）：串口 JSON 观测流 schema —— 事件类型、字段、Observer 证据口径。
// 纯函数部分可直接验证（观察流行解析器参照）；固件侧 emit 覆盖为 E5 真机项。
...
test('固件侧串口事件 emit 静态探测（可 FAIL=差距）', () => {
  ...
  const evHit = serial.event_types.filter((e) => fw.includes(`"${e}"`) || fw.includes(`'${e}'`) || fw.includes(`event.*${e}`) || fw.includes(`"${e}\\`));
