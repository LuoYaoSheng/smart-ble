...
> 报告日期：2026-09-21（P5 最终交付轮）。报告基线：`a1215a8` + 本轮看板/证据提交（准确哈希以看板行为准）。
...
| WIN-012 F-WIN 立项 | PASS_WITH_OBS | 20260920 | `20260920-FWIN-GWIN/flutter/` | windows runner + FBP 1.36.8/winrt 0.0.20；真机扫描 3 台含 SHID；P002 扫码 Windows 降级（粘贴单路径）。P5 终报补记：Windows 侧 GATT 连接链无独立真机走查（§3/§7） |
...
| 连接+服务枚举 | ✔ | ✔ | ✔ | ◐ | ✔ | ✔ | 20260918-WIN-ALL；Q 本轮 1svc/3char；F=§7 缺口 |
| 读（INFO 身份） | ✔ | ✔ | ✔ | ✔ᵐ | ✔ | ✔ | Q 本轮 130B JSON fw1.2.0 |
| 写（带响应=PARITY-007） | ✔ | ✔ | ✔ | ✔ᵐ | ✔¹ | ✔² | 20260918 写探针矩阵；注¹²见下 |
| Notify 订阅/退订 | ✔ | ✔ | ✔ | ✔ᵐ | ✔ | ✔ | Q 本轮 STATUS 订阅语义断言 |
...
- F-WIN Windows 侧 GATT 连接链无独立真机走查（本轮终报新登记；扫描已真机、GATT 与移动端同源但 FBP winrt 后端是独立原生路径）——建议并入下一真机窗口顺带走查。
- F 页面内深层（P006 双栏、P002 系）无自动化缝，未做像素级探针（UIALIGN-PAGE §7）；功能与移动端同源已验。
