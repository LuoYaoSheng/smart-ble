// P3 · WIN-008 E-WIN OTA 全链（选包→start→ready→DATA→commit→success→reboot）
...
// 2. 连接 BLEToolkit-Server
const conn = await ev(`(() => {
  const cards = [...document.querySelectorAll('device-card')];
  const t = cards.find(c => (c.device?.name || '').includes('BLEToolkit'));
  if (!t) return 'NO_CARD: ' + cards.map(c => c.device?.name).join('|');
  const b = t.querySelector('#connectBtn');
...
// 3. 打开 OTA 对话框（右上 dl 图标按钮 tooltip=OTA 固件升级）
const otaOpen = await ev(`(() => {
  const btns = [...document.querySelectorAll('button')];
  const b = btns.find(x => (x.getAttribute('aria-label') || '').includes('OTA') || (x.querySelector('use') && x.querySelector('use').getAttribute('href') === '#i-dl' && x.closest('.subnav, header, .navbar')));
  if (!b) return 'NO_OTA_BTN';
  b.click(); return 'OTA_OPENED';
})()`);
console.log('ota-open:', otaOpen.result.value);
await sleep(1200);
await shot('ota-01-dialog');

// 4. 注入文件（DOM.setFileInputFiles）
...
await sleep(1500);
await shot('ota-02-file-selected');
...
if (start.result.value !== 'STARTED') { await shot('ota-02b-start-blocked'); process.exit(1); }
...
for (let i = 0; i < 300; i++) {
  ...
  if (text !== last) {
    console.log(`[${i * 2}s]`, text);
    last = text;
  }
  if (/成功|完成|success|重启/.test(text)) {
    await shot('ota-03-success');
    console.log('OTA_FLOW=SUCCESS');
    process.exit(0);
  }
  if (/失败|错误|fault|error/i.test(text)) {
    await shot('ota-03-failed');
    console.log('OTA_FLOW=FAILED');
    process.exit(2);
  }
}
await shot('ota-04-timeout');
console.log('OTA_FLOW=TIMEOUT');
