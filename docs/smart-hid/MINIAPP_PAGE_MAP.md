...
**页面内容**：logo + 应用名 + 版本号 + 简介 + 技术栈标签；「当前环境」（平台/系统版本/设备型号）；功能特性 chips；支持平台 chips；「相关链接」菜单（官方网站 / 问题反馈 / 版本记录）；分享。（「更多小程序」推广卡 2026-09-10 移除）
...
- **死按钮**：无——官网/反馈在微信端按合规降级为复制链接（有意设计）；「更多小程序」推广卡已移除（2026-09-10）。
