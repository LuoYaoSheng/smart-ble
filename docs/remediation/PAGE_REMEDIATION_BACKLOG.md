...
| P1 | PAGE-006 | `RUNTIME-GATT-CODEC-001` | HEX write codec 缺失 |
| P1 | PAGE-006 | `RUNTIME-WRITE-QUEUE-001` | write-queue 缺失 |
| P1 | PAGE-006 | `RUNTIME-RECONNECT-001` | reconnect-policy 缺失 |
| P1 | PAGE-006 | `RUNTIME-CONNECTION-DISCOVERY-001` | connectDevice 发现编排（TEST-I-003） |
| P1 | PAGE-001 | `RUNTIME-FILTER-001` | device-filter 关键词命中 |
| P1 | PAGE-001 | `RUNTIME-DISPLAY-NAME-001` | display-name.js 缺失 |
| P1 | PAGE-007 | `RUNTIME-SESSION-001` | Registry 配网会话排除 + subscription_count |
...
- 主 Fix：`RUNTIME-FILTER-001`（root=RC-DEVICE-FILTER）
- 说明：Fake Runtime E4 PASS；产品扫描过滤/显示名/时长未对齐目标
- 依赖策略：页面表象失败优先引用 **RUNTIME_*** / **OTA_*** / **ESP32_***；仅架构归属页面时用 PAGE-*（如 PAGE-BROADCAST-001）

### PAGE-002

- 产品结论：**FAIL**
- E4 case：PASS=0 FAIL=0 BLOCKED=0 NOT_IMPLEMENTED=29
- 主 Fix：`RUNTIME-LOG-REDACTION-001`（root=RC-LOG-REDACTION）
- 说明：hid/add + use-smart-hid-provisioning 存在；Workflow 被 Runtime/桥接断点阻断
- 依赖策略：页面表象失败优先引用 **RUNTIME_*** / **OTA_*** / **ESP32_***；仅架构归属页面时用 PAGE-*（如 PAGE-BROADCAST-001）
...
- 产品结论：**FAIL**
- E4 case：PASS=0 FAIL=30 BLOCKED=0 NOT_IMPLEMENTED=0
- 主 Fix：`RUNTIME-GATT-CODEC-001`（root=RC-GATT-HEX）
- 说明：device/detail 存在；GATT/OTA/重连断点导致产品 FAIL
- 依赖策略：页面表象失败优先引用 **RUNTIME_*** / **OTA_*** / **ESP32_***；仅架构归属页面时用 PAGE-*（如 PAGE-BROADCAST-001）

### PAGE-007

- 产品结论：**FAIL**
- E4 case：PASS=0 FAIL=14 BLOCKED=0 NOT_IMPLEMENTED=0
- 主 Fix：`RUNTIME-SESSION-001`（root=RC-SESSION-REGISTRY）
- 说明：connected/index 存在；Session 口径未对齐
- 依赖策略：页面表象失败优先引用 **RUNTIME_*** / **OTA_*** / **ESP32_***；仅架构归属页面时用 PAGE-*（如 PAGE-BROADCAST-001）

### PAGE-008

- 产品结论：**FAIL**
- E4 case：PASS=0 FAIL=23 BLOCKED=2 NOT_IMPLEMENTED=0
