...
- **v1.0.8（2026-09-04 · 用户走查修正：P009 关于页去大横幅，「功能第一」原则落地）**：用户在双手机真机走查（F-AND/U-AND）后提出：原型与实现都要**功能第一，不为展示而展示**——P009 顶部品牌大横幅（`brandcard`：全宽渐变底 + 130px 装饰圆 + 24px display 大字）属纯展示元素，收缩为**紧凑品牌行**（38px 图标 + 名称 + `v版本 · 通道 · 零后端 · 零本地持久化` 单行元数据，标准 card 容器）；版本/通道元数据保留（F027），推广列表/应用信息/菜单行/页脚零改动。`pages.css` 删除 `.brandcard` 全套 5 条规则（改后内核与三壳零引用，web 壳独立 CSS 不含 p009 无引用）。同轮盘点其余页面：P008 广播顶部状态本就是导航栏小徽章 + 表单卡（操作优先，不改）；P003/P005/P006/P007/P010 均无同类纯展示大块。实现侧对齐差距与整改另行登记：06_review/PROTOTYPE_ALIGNMENT_AUDIT.md。内核（p009-about.js + pages.css）字节重同步三壳（戳 → 三壳 `?v=1.4.7`，本壳 `?v=1.1.8`；web 无 P009 不涉及）。验证：浏览器真实渲染 P009 正常、console 0 error、无 `.brandcard` 残留节点。
