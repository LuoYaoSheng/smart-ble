...
| P007 | 已连接 | 活动 Session、单断、全断、部分失败、Profile 分流 | NOT_RUN | E5 双机 PASS_WITH_LIMITATION（列表 N=2/单断→N=1/重连→N=2/全断→空态；部分失败弹窗未触发） | NOT_RUN | f013-dualphone/f013-summary.md + uand-f013b-all-disconnect-results.json + 截图 | U-AND PASS_WITH_LIMITATION（E5+ESP32+华为广播者双机布景，WIN-UAND-001 在册）；页面级 20 项逐查留页面全量回归 |
