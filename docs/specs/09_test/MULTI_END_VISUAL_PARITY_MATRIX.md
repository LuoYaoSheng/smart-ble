...
> 2026-09-11 假数据通道补充：U 线（U-WX/U-AND 同码）九页 43 态已可离线渲染采集（H5 dev + `?mock=1`，正典演示集驱动真实渲染管线；43/43 截图 + 36/36 文案断言，见 §3 末行与 [REPORT](../../../verification/windows-mobile-v1/20260911-h5-mock-sweep/REPORT.md)）。该口径为 **code-level** 证据，真机六态 VISUAL_PASS 判定仍以目标端运行态截图为准。
...
| P007 | 汇总卡/ON 头标/空态两文案 | NOT_AUDITED | NOT_AUDITED | NOT_AUDITED | UI-G2 结构对齐（DeviceCard conn 变体+AppEmpty+sumcard）；2026-09-10 烟测默认态（已连接空态+占位设备卡）三线+Apple 同构截图落袋（§3）；vis1 N-MAC 设备页空态（「当前没有已连接设备/先在扫描页完成设备连接/去扫描」）落袋（§3 vis1-nmac）；**vis1 F-MAC 空态同三行逐字一致+汇总卡在位（§3 vis1-fmac）**；**2026-09-11 vis1-uand：U-AND 空态首证（§3）** |
| P008 | 徽章六值+N/31 字节提示+超限红字+日志面板 | NOT_AUDITED | NOT_AUDITED | NOT_AUDITED | UI-G2 结构对齐（AppNavbar PERIPHERAL+AppBadge 三态+LogPanel card）；2026-09-10 UUID 黄条深字收敛 --c-warning-deep；烟测默认态（未开启+日志空卡）三线+Apple 截图落袋（§3）；**vis1 N-MAC 广播中真实态：徽章「广播中」+6/31 字节+adv_start/广播已开启 日志行（真实 CoreBluetooth 外设、唯一可达线；采后即停恢复射频）（§3 vis1-nmac）**；**vis1 F-MAC 未开启+广播中真实态（flutter_ble_peripheral 真实启停：本地名 BLE Toolkit+/Service UUID 大写/0/31 字节 → 徽章「广播中」+8/31 字节+「停止广播」，采后即停恢复射频）——F 线 Mac 真实外设态（§3 vis1-fmac）**；**2026-09-11 vis1-uand：U-AND 未开启态（PERIPHERAL 徽章+四字段表单+开始广播钮+空日志面板）首证（§3）** |
| P009 | 品牌卡+信息卡+菜单 4 项+页脚（推广卡 2026-09-10 随 F028 移除） | NOT_AUDITED | NOT_AUDITED | NOT_AUDITED | UI-G2 结构对齐（AppNavbar ABOUT+版本 chip+品牌渐变挂 token）；烟测默认态三线+Apple 截图落袋（§3）；推广区移除后 P009 截图已三线重采（§3 重采行）；**2026-09-10 vis1 F-MAC 对照发现品牌副标题缺渠道段（实拍「v1.0.5+101 · 零后端 · 零本地持久化」vs 正典三段式「v… · preview · …」）——同日修复补渠道段，复采逐字命中「v1.0.5+101 · preview · 零后端 · 零本地持久化」（§3 vis1-fmac；品牌名/菜单/页脚其余对齐）**；**2026-09-11 vis1-uand：U-AND 品牌卡（含渠道段）+四菜单项首证（§3）** |
| P010 | 版本卡+限制卡+历史/预览卡+页脚声明 | NOT_AUDITED | NOT_AUDITED | NOT_AUDITED | UI-G2 结构对齐（AppSubnav+AppEmpty doc×2）；vis1 可达态：**四线**默认态截图（N-IOS simctl / U-WX automator reLaunch / F-AND 宿主窗口 / N-MAC AX 通道），四卡结构+版本 1.0.5（构建 101）+渠道 preview+tag 逐字一致（§3 vis1-*）；**vis1 F-MAC 第五线同值一致（版本卡 1.0.5/build 101/preview）（§3 vis1-fmac）**；**2026-09-11 vis1-uand：U-AND 版本卡同值一致（§3）** |
...
| 2026-09-11 | 20260911-h5-mock-sweep（**假数据通道·非视觉 Gate**） | 九页 43 态（P001×8/P002×10/P003×2/P005×6/P006×5/P007×4/P008×6/P009/P010） | — | **U 线 code-level（H5 载体）**：uniapp 同码 H5 dev + ?mock=1 桥（services/mock/ 正典演示集驱动真实渲染管线），43/43 截图 + 36/36 正典文案断言 （[REPORT.md](../../../verification/windows-mobile-v1/20260911-h5-mock-sweep/REPORT.md)） | 同 U 线（H5 载体不折算 F-AND） | — | — | — | 口径：不折算真机 VISUAL_PASS；修复 UI-DEF-01（DeviceCard displayName 链）/02（eb-title fs-h2）/03（P003 名 fs-h1）；M5 phase-2 门禁（阴影白名单/navbar 域/wrapper 32rpx/regFS 圈定）全绿 |
| 2026-09-11 | ui_six_state_copy_test（**F-AND 文案级·非视觉 Gate**） | P001 五态+P007/P008/P009/P010 | — | — | — | **9/9 PASS（provider override 驱动，含 UI-DEF-05 ESP32 Profile 补注册 + UI-DEF-06 按钮行溢出修复）** | — | — | 与 U 线 copycheck 同口径；空态B/扫描完成受 _hasScanned 门控留 F-MAC 真扫描（vis1 已有）；golden 无 CJK 资产未做 |
| 2026-09-11 | 20260911-uand-vis1（**U-AND 模拟器可达态·HBuilderX 域首开**） | P001 默认/空态A/扫描中/扫描完成/筛选 + P002 默认（--pagePath 深链）+ P007 空态 + P008 未开启 + P009 + P010 + 基座权限弹窗 | — | — | **uand-p001-{default,empty-a,scanning,scan-done,filter} / uand-p002-default / uand-p007-devices-empty / uand-p008-off / uand-p009-about / uand-p010-versions / uand-sys-permission-dialog（12 张）**（HBuilderX 5.24 CLI 标准基座→Pixel_API35 模拟器；真实扫描启停：「扫描中 · 5s 会话」→「扫描完成 · 发现 0 台」+空态B） | — | — | — | U-AND 自 09-09 UI-G1 E5 真机后首次复证、**首份非 P001 页面像素证据**；bundle 文案核对 113/121 硬命中（8 条 H5-mock 专有按设计缺席；「未发现可用服务」=未引用导出 tree-shake）；OCR 幻觉两起经源码+bundle grep 裁定非缺陷（详见 [REPORT](../../../verification/windows-mobile-v1/20260911-uand-vis1/REPORT.md)）；基座噪声弹窗（标识码/CALL_PHONE）正式打包后不复现 |
| 2026-09-11 | 20260911-fmac-deep（**F-MAC 深态轮·受阻裁定+三修复**） | P001 扫描结果+SHID 卡 + P002 step1/step2（真夹具链） | — | — | — | — | **F-MAC（Release 直启 800×632 真窗）：fmac-p001-scan-result / fmac-p001-shid-card / fmac-p002-step1-connecting / fmac-p002-step2-form（4 帧，均中性裁剪复核）** | — | — | 深链（step3/P003/P005）受阻于 flutter-test 实例看不见本机软件夹具广播（环境怪癖，详见 [REPORT](../../../verification/windows-mobile-v1/20260911-fmac-deep/REPORT.md)§3）；**本轮修复 3 项 F 线缺陷：相机 entitlement 缺失（DEF-01，发布级配网死锁）/ P005 offline 态缺失（DEF-02，PAGE_SPEC:326）/ shidConfigureBtn 键歧义（DEF-03，E2E 定位）**；analyze 0 + test 122/122；新增 fmac_deep_capture_test.dart 采集基建 |
| — | **视觉 Gate 六态全量（六态×9 页×三端+Apple）** | — | — | — | — | — | — | — | vis1 可达态首轮已落袋（上行 10 张：空态A×3/筛选×2/Error 横幅×1/P010×3/P002×1）；**U-AND HBuilderX 域已打通（20260911-uand-vis1，12 张）**；**F-MAC 深态已通至 P002 step2（20260911-fmac-deep，4 帧+3 修复），step3/P003/P005 待续走**；剩余待：扫描中/设备列表/权限弹窗/连接态页（P003/P005/P006）需 BLE 夹具或真机、U-WX 扫描态需元素级驱动方案（或 devtools 真机预览） |
