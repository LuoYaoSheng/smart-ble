...
| STATE-P008-03 | 不支持 | 探测失败/H5/插件缺失 | UNSUPPORTED 说明 | 浏览 | 平台变化 | 无 | TEST-P-008 |
...
| STATE-P008-09 | 蓝牙关闭 | 适配器 off | 错误+去开启 | 恢复 | 就绪 | 无 |  |
...
TEST-A-010、TEST-E-006（Observer 字节级一致 + 停止后消失）。发布条件：Observer E5 存在，否则本能力只能 PREVIEW（DEC-002）。
