...
Smart HID 已配设备的**唯一完整列表**：查看、移除本机记录，并进入详情/重配/诊断。是 Smart HID 历史 SSOT 的 UI 面，也是**历史管理的唯一宿主**（删除/批量/TTL/重命名只在本页，FEAT-061；PAGE-001 紧凑入口只读，TP-G0-R1 冻结）。

存在必要性：PAGE-001 只保留只读紧凑入口；完整列表、TTL 说明与移除管理必须有一个唯一宿主，避免两处列表互相漂移。
