...
4. Smart HID 历史紧凑区（有历史时）：最近 1 台 + "全部历史"入口（形态 DEC-012）；
...
| 权限状态 | enum(granted/denied/permanent-denied/na) | 权限 API | 实时 |
...
| OP-P001-12 | 历史紧凑-移除 | 同上 | 无 | 确认框 | 删除本机记录 | toast+紧凑区更新 | ERR-DATA-01 toast | 无 | 无 | TEST-P-004 |
...
| 微信 | Full/Adapted；定位权限策略（FEAT-006、DEC-003）；广播字段可能不全，缺失标"平台未提供" |
...
扫描 UI 合并刷新 ≤1s（NFR-004）；列表上限 100（NFR-006）；过滤 ≤50ms（NFR-007）；默认扫描时长 DEC-013（推荐 5s 常量可调）；状态变化到 UI ≤1s（NFR-003）。
...
- [x] 五个独立状态（权限拒绝/蓝牙关/扫描失败/无设备/筛选空）在页面与测试中互相可区分；
- [x] N 与 M 同时可见；
- [x] 显示名解析链完整生效；
- [x] 全部 15 个操作有 ID、反馈与清理定义；
- [x] 权限错误与扫描错误不合并。
...
REQ-005~018｜FEAT-005~019｜FLOW-001/002/003/010｜错误：ERR-PERM-01/03、ERR-BT-01/02/03、ERR-SCAN-01、ERR-SYS-03、ERR-DATA-01/03｜数据：DATA-001/002｜[`05 总表`](../05_TARGET_PAGE_CATALOG.md)｜[`06 流程`](../06_TARGET_USER_FLOWS.md)｜[`10 Runtime`](../10_RUNTIME_ARCHITECTURE_AND_RESOURCE_OWNERSHIP.md)
