...
2.  **Start Scan**: Click `[🔍 开始扫描]`. Verify the `Dummy-BLE-01` card appears within 1 second.
3.  **UI Filters**: 
    - Set the "名称前缀" filter to `Dummy`. Verify the card stays.
    - Set the "名称前缀" filter to `ESP`. Verify the list becomes empty.
    - Clear the filter.
4.  **Connect Device**: Click `[连接]` on the `Dummy-BLE-01` card.
...
5.  **Multi-Device Tabs**:
    - Assert the `[已连接设备]` (Connected Devices) tab displays a badge of `1`.
6.  **Disconnect**:
    - Click `[断开]`. Assert the Connected Devices tab displays "暂无已连接设备" (No connected devices empty state).
