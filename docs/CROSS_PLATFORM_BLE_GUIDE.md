...
### Tauri (Rust / btleplug) — 已升级为事件驱动

**v1（旧）**：每 500ms 主动 `GATT Read`，伪模拟通知。缺陷：数据丢失 + 额外射频开销。

**v2（当前）**：`subscribe()` 后，通过 `peripheral.notifications()` 获取专属 stream，仅在硬件真实 push 时触发，完全符合 BLE 规范：
...
tokio::spawn(async move {
    ...
    // ValueNotification { uuid: Uuid, value: Vec<u8> }
    ...
    // Stream ends when peripheral disconnects or unsubscribes
});
```

> **注意**：btleplug 0.11.8 的 `CentralEvent` 枚举**不包含** `ValueNotification` 变体。通知值只能通过 `Peripheral::notifications()` 独立 stream 获取，不要从 `Central::events()` 流尝试捕获。

**Handle 存储 Key 格式**：`"deviceId::charUuid"`，支持同一设备多个特征值同时订阅，断开时按前缀批量清理。

### Electron (Node.js / noble)

通过 `characteristic.on('data', ...)` 监听真实 push 事件，不使用轮询。
